...
Assert-Match -Actual $buildScriptText -Pattern 'Get-ReleaseInventory' -Message 'Build script should enumerate actual GitHub releases before deriving metadata.'
Assert-Match -Actual $buildScriptText -Pattern 'Find-LatestReleaseAsset' -Message 'Build script should select metadata from a published release asset rather than a branch version.'
Assert-Match -Actual $buildScriptText -Pattern 'ExistingEntry' -Message 'Build script should retain existing metadata if no valid release asset is available.'
Assert-Match -Actual $buildScriptText -Pattern 'Preserving the existing repository entry' -Message 'Build script should explicitly preserve an existing entry during an incomplete release.'
Assert-Match -Actual $buildScriptText -Pattern '\[bool\]\s*\$_\.draft' -Message 'Build script should exclude draft releases from repository metadata.'
Assert-Match -Actual $buildScriptText -Pattern '\[bool\]\s*\$_\.prerelease' -Message 'Build script should exclude prereleases from repository metadata.'
Assert-Match -Actual $buildScriptText -Pattern 'Sort-Object\s+-Descending' -Message 'Build script should select the latest published stable release deterministically.'
Assert-Match -Actual $buildScriptText -Pattern 'Ref \$tagName' -Message 'Build script should read manifest metadata at the released tag.'
